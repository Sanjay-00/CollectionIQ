...
- 358 automated tests cover every pandas/business-logic path (bucketing, KPI computation, the 7-tier priority framework, bucket migration, the compiler/registry layer, every report section) with zero live LLM calls, so the full regression suite runs in seconds.
- Each plain-English query resolves through 2 sequential LLM calls (occasionally a 3rd repair call on a compile error); choosing Gemini 2.5 Flash-Lite over a heavier model keeps total query latency in the single-digit-second range even under concurrent users.
